...
<?
	...
	$regist_day = date("Y-m-d (H:i)");  // 현재의 '년-월-일-시-분'을 저장
		...
		$upfile_tmp_name = $_FILES["upfile"]["tmp_name"];	// 파일의 임시 파일 이름(tmp1.,jpg)
		$upfile_type     = $_FILES["upfile"]["type"];	// 첨부파일의 종류
		$upfile_size     = $_FILES["upfile"]["size"];	// 첨부파일의 실제 용량
		$upfile_error    = $_FILES["upfile"]["error"];	// 업로드 상태(true, false);
		*/

	// 다중 파일 업로드
	$files = $_FILES["upfile"]; // 배열로 리턴해준다
	$count = count($files["name"]); //  배열의 개수 뽑기
	...
	for ($i=0; $i<$count; $i++)
	{
		...
		$upfile_type[$i]     = $files["type"][$i]; // image/jpeg, image/pjppeg
		$upfile_size[$i]     = $files["size"][$i]; // 22.3kb
		$upfile_error[$i]    = $files["error"][$i]; // true;
      
		$file = explode(".", $upfile_name[$i]); // 에러가 없으면
		$file_name = $file[0];
		$file_ext  = $file[1];
