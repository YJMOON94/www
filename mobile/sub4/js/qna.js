...
$('.qna .article:first').find('.answer').slideDown('slow');
...
$('.all').toggle(function(e){
    ...
    $(this).css('background','url(./images/down.png)no-repeat center center').css('background-size','cover');
},function(e){
    e.preventDefault(); 
    article.find('.answer').slideUp(500);
    article.removeClass('show').addClass('hide');
    $('.all .hidden').text('모두열기');
    $(this).css('background','url(./images/up.png)no-repeat center center').css('background-size','cover');
});